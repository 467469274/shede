<template>
  <div class="orderDetail">
    <el-form label-width="100px">
      <el-form-item label="">
        <p class="orderTitle">订单状态：{{type}} <span>订单编号:{{ddbj}}&nbsp;&nbsp;下单时间:{{xdsj}} &nbsp; &nbsp; &nbsp;购买用户名称:{{userName}}</span></p>
        <p><a>订单基本信息</a>    >>>>>    <a :href="url">订单基本信息</a></p>
      </el-form-item>
      <el-form-item label="备注:" v-if="yType!=0 ||yType!=11">
        <el-input
          type="textarea"
          :rows="2"
          placeholder="请输入内容"
        ...
        <el-button type="primary" style="margin-left: 20px">保存备注</el-button>
      </el-form-item>
      <el-form-item label="押金退款:" v-if="yType==0">{{yjtk}}
      </el-form-item>
      ...
      <el-form-item label="租金退款:" v-if="yType==0">{{zjtk}}
      </el-form-item>
      ...
      <el-form-item label="修改状态:" v-if="yType==1">
        <el-button type="primary" style="margin-right: 20px" @click="closeDD">关闭订单</el-button>
        <el-button type="primary" style="margin-left: 20px" @click="sure">变更为待发货订单</el-button>
      </el-form-item>
      <el-form-item label="修改状态:" v-if="yType==4">
        <el-button type="primary" style="margin-right: 20px" @click="closeDD">关闭订单</el-button>
        <el-button type="primary" style="margin-left: 20px" @click="sure('isSure')">变更为已发货订单</el-button>
      </el-form-item>
      ...
      <el-form-item label="" v-for="(item ,index) in get_remark" :key="index">
        <p>{{item.remark}}</p>
      </el-form-item>
      <el-form-item label="费用明细:">
        <p v-if="ist"> 租金：应付【￥<el-button type="text" @click="textPop('zujinyingfu')">{{zujinyingfu}}</el-button>】 / 实收￥{{zujinshishou}}</p>
        <p v-if="!ist"> 租金：应付【￥{{zujinyingfu}}】 / 实收￥{{zujinshishou}}</p>

        <p v-if="ist">  押金：应付【￥<el-button type="text" @click="textPop('yajinyingfu')">{{yajinyingfu}}</el-button>】 / 实收￥{{yajinshishou}}</p>
      ...
      <el-form-item label="商品发货:" class="formP" v-if="yType==2 || yType==3 || yType==4">
        ...
        <p>
          物流单号:&nbsp;&nbsp;&nbsp;<el-input v-model="wldh" placeholder="请输入内容" style="width: 235px"></el-input>
        </p>
        <p>
          客服人员:&nbsp;&nbsp;&nbsp;<el-input v-model="kfry" placeholder="请输入内容" style="width: 235px"></el-input>
        </p>
      </el-form-item>
      <el-form-item label="商品发货:" class="formP" v-if="yType==5 || yType==7 || yType==8|| yType==9 || yType==10 || yType==11 || yType==0">
        <p><span>发货日期:&nbsp;&nbsp;&nbsp;{{fhrq}}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span>收到日期:{{sdrq}}</span></p>
        <p><span>目标地址:&nbsp;&nbsp;&nbsp;{{shr}}&nbsp;&nbsp;&nbsp;{{mobile}}{{mbdz}}</span></p>
        <p>发送网点:&nbsp;&nbsp;&nbsp;{{choseWd}}
        </p>
      ...
      <el-form-item label="用户返还:" class="formP" v-if="yType==2 || yType==3 || yType==4 || yType==5 || yType==7">
        ...
        <p>
          物流单号:&nbsp;&nbsp;&nbsp;<el-input v-model="wldh2" placeholder="请输入内容" style="width: 235px"></el-input>
        </p>
      </el-form-item>
      ...
      <el-form-item label="商品全家福" v-if="yType!=2 || yTpe!=3 || yType!=4 || yType!=5">
    ...
    methods:{
      getData(obj){
        ...
        _this.postFetch('/admin/order/detail',obj,function(data){
          ...
          _this.wldh = data.data.back_odd_number;
          ...
          _this.post_odd_number = data.data.wldh;
